...
/**
 * The self logic center — a reasoning pass that sits between the classifier's
 ...
 *
 * <ol>
 *   <li><b>Is this actually advertising?</b> An advert needs somewhere to go —
 ...
 *       still requires real identifiers — PatternPack owns that check — but a
...
public final class LogicCenter {
    ...
    public static Verdict evaluate(String raw, String label, double confidence, boolean hasAddress) {
        return evaluate(raw, label, confidence, hasAddress, java.util.List.of());
    }

    /**
     * Context-aware evaluation. The player's recent messages change two calls:
     *
     * <ul>
     *   <li><b>Sarcasm carries over.</b> "yeah right lol" followed by something
     *       slur-shaped but unaimed is banter continuing, not an attack starting.
     *       A slur with nobody targeted inside a joking thread downgrades to
     *       review instead of punish.</li>
     *   <li><b>Adverts accumulate.</b> A campaign spread over short messages —
     *       "new smp", "no lag", "dm me" — looks innocent alone and obvious in
     *       sequence. If earlier messages contain invite verbs, an advert-shape
     *       verdict proceeds even without a parseable address.</li>
     * </ul>
     */
    public static Verdict evaluate(String raw, String label, double confidence,
                                   boolean hasAddress, List<String> prevMessages) {
        ...
        if (prevMessages != null) {
            for (String prev : prevMessages) {
                if (prev != null && !prev.isBlank()) prior.append(prev.toLowerCase(Locale.ROOT)).append(' ');
            }
        }
        String history = prior.toString();
        boolean sarcastic = SARCASM_MARKERS.stream().anyMatch(s::contains)
                || SARCASM_MARKERS.stream().anyMatch(history::contains);
        ...
        boolean priorInviteShaped = INVITE_VERBS.stream().anyMatch(history::contains);

        switch (label) {
            case ChatGuardModule.CAT_ADVERT, ChatGuardModule.CAT_LIGHT_ADVERT -> {
                // No address anywhere — but an advert campaign leaves invite
                // verbs in the history, so a campaign spread over short
                // messages still proceeds.
                ...
                if (sarcastic && !hasAddress && !priorInviteShaped) {
     ...
     * layers — it only tells the logic centre whether the message contains
