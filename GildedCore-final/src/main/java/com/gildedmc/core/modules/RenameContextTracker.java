...
public final class RenameContextTracker implements Listener {
    ...
    public int addressRenameCount(UUID playerId) {
        ...
        synchronized (history) {
            ...
            for (RenameEntry e : history) {
                if (ChatGuardModule.looksLikeAddress(e.newName)) count++;
            }
            return count;
        }
    }
        ...
        synchronized (history) {
            ...
            for (RenameEntry e : history) {
                if (now - e.timestamp < RENAME_TTL_MS) {
                    totalCount++;
                    if (ChatGuardModule.looksLikeAddress(e.newName)) addressCount++;
                }
            }
     ...
     *
     * <p>Reads the bounded, TTL'd rename history rather than a separate
     * unbounded pattern set (removed): same signal, at most 30 trigram
     * comparisons, and entries expire with the history.
     */
    public boolean isRepeatedRename(UUID playerId, String newName) {
        Deque<RenameEntry> history = renameHistory.get(playerId);
        if (history == null || newName == null) return false;
        long now = System.currentTimeMillis();
        synchronized (history) {
            for (RenameEntry e : history) {
                if (now - e.timestamp >= RENAME_TTL_MS) continue;
                if (similarity(e.newName, newName) >= 0.75D) return true;
            }
        }
        return false;
    }
