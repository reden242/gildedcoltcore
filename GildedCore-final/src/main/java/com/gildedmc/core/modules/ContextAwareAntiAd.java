 ...
 *       ({@link LocalContextAggregator}).</li>
 * </ol>
 *
...
public final class ContextAwareAntiAd {
    ...
    public AdVerdict checkChat(Player player, String raw) {
        ...
        if (l2Score >= L2_BLOCK && ChatGuardModule.addressPresent(raw)) {
            // L5 must not be the one path that blocks a message with no address in
            // it. Every other layer reaches its block only after advertHit or
            // AdvertContext found something; this rule previously did not, so a
            // first-token-dominant classifier could block "join us" on score alone.
            shouldBlock = true;
    ...
    public AdVerdict checkBook(Player player, String text) {
        ...
        if (l2Score >= L2_BLOCK && ChatGuardModule.addressPresent(text)) {
            // Same evidence bar as checkChat (F2): no address, no L2-only block.
            return maybeDowngrade(AdVerdict.block(combined,
